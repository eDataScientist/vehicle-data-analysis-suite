...
from scrips.services.gcc_presence_service import GCCPresenceService, GEMINI_AVAILABLE
...
def gcc_presence_checker_tab():
    """GCC Presence Checker workflow tab"""
    st.markdown(
        """
    <div class="workflow-card">
        <h3>🌍 GCC Presence Checker</h3>
        <p>Uses Gemini AI with Google Search grounding to verify if unmatched vehicle makes, models, or trims exist in the GCC market.</p>
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    if not GEMINI_AVAILABLE:
        st.error(
            "❌ google-genai package is not installed or GEMINI_API_KEY is missing. Please install it and add the API key to your .env file to use this feature."
        )
        return
    ...
    st.subheader("📄 Unmatched Data File")
    st.info("Upload an unmatched makes, models, or trims file generated by the Spec Mapper.")

    uploaded_file = st.file_uploader(
        "Upload Unmatched Vehicles CSV/Excel",
        type=["csv", "xlsx", "xls"],
        key="gcc_upload",
    )
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            st.success(f"✅ Loaded {len(df)} unmatched vehicle records")
            
            st.write("**Data Preview:**")
            safe_display_dataframe(df, 5, "Unmatched Data Preview")
            
            # Show configuration and run button
            st.subheader("⚙️ Verification Settings")
            score_threshold = st.slider("Skip threshold (Score >= this will be flagged as BELOW_THRESHOLD)", min_value=1, max_value=100, value=60)
            
            if st.button("🚀 Run GCC Verification", type="primary"):
                # Run the async verification
                service = GCCPresenceService(score_threshold=score_threshold)
                ...
                def update_progress(current: int, total: int, status_msg: str):
                    ...
                    status_text.text(f"Progress: {current}/{total} - {status_msg}")
                ...
                with st.spinner("Connecting to Gemini AI..."):
                    ...
                    try:
                        result_df = loop.run_until_complete(
                            service.process_batch(df, progress_callback=update_progress)
                        )
                    finally:
                        loop.close()
                
                st.success("✅ GCC Presence Verification complete!")
                ...
                safe_display_dataframe(result_df, 10, "Processed Data")
                ...
                create_download_link(
                    output, 
                    f"{original_name}_gcc_verified.csv", 
    ...
    if reference_file is not None and logs_file is not None:
        ...
        if True:  # Translation is now always available
            ...
            if TRANSLATION_AVAILABLE:
                ...
                st.info(
                    "💡 Translation feature requires google-genai library. Install it to enable translation."
                )
...
def translation_service_tab():
    ...
    if not TRANSLATION_SERVICE_AVAILABLE:
        st.error(
            "❌ Translation service not available. Please install the google-genai library."
        )
            ...
            if enable_translation:
                if st.button(
                    ...
                    with st.spinner("Translating Arabic text..."):
                        try:
                            result = asyncio.run(
                                analyzer.perform_translation(api_key=None, progress_callback=update_progress)
                            )
...
def main():
    ...
    if selected_workflow == "🏠 Home":
        ...
        ### 🌍 GCC Presence Checker
        - **Purpose**: Verify if unmatched vehicle makes, models, or trims exist in the GCC market.
        - **Features**: Uses Gemini AI with Google Search grounding, automated categorization (Sold, Different Name, Not in GCC), pre-search filtering and thresholding.
        - **Output**: Processed data with GCC presence classification.
        ...
        - **Features**: Multi-column selection for translation with progress tracking and caching
    ...
    elif selected_workflow == "🌍 GCC Presence Checker":
        st.sidebar.markdown("""
        **GCC Presence Checker** verifies vehicle availability with:
        - Gemini AI and Google Search grounding
        - Automated categorization (Sold, Different Name, Not in GCC)
        - Pre-search filtering and thresholding
        - Works natively with Spec Mapper outputs
        """)
        gcc_presence_checker_tab()
